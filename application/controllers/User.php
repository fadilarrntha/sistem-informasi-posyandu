<?php
defined('BASEPATH') or exit('No direct script access allowed');

class User extends CI_Controller
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('User_model');
    }

    public function index()
    {
        $data['title'] = 'Data Akun Pengguna | Posyandu Paleto Indah';
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        
        $data['users'] = $this->User_model->getDataUser();

        $this->load->view('templates/header-form', $data);
        $this->load->view('templates/sidebar-admin');
        $this->load->view('templates/topbar-admin', $data);
        $this->load->view('user/index');
        $this->load->view('templates/footer-form');
    }

    // MULAI PROFILE USER PETUGAS
    public function profile()
    {
        $data['title'] = 'Profile | Posyandu Paleto Indah';
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();

        $id_users = $this->input->post('id_users');
        $username = $this->input->post('username');
        $upload_image = $_FILES['image']['name'];
        $currpassword = $this->input->post('current-password');
        $newpassword = $this->input->post('new-password');
        $repeatpassword = $this->input->post('repeat-password');
        if ($upload_image) {
            $config['allowed_types'] = 'gif|jpg|jpeg|png';
            $config['max_size'] = '2048';
            $config['upload_path'] = './build/img/profile';

            $this->load->library('upload', $config);
            if ($this->upload->do_upload('image')) {
                $old_image = $data['user']['image'];
                if ($old_image != 'icon-user.png') {
                    unlink(FCPATH . 'build/img/profile/' . $old_image);
                }

                $new_image = $this->upload->data('file_name');
                // $coba = array(
                //     'image' -> 
                // )
                $this->db->set('image', $new_image);
            } else {
                echo $this->upload->display_errors();
            }
        }
        $pass = $data['user']['password'];

        if ($currpassword !== '') {
            if ($currpassword != $pass) {
                $this->session->set_flashdata('psn', 'Current Password Salah!');
            } else {
                if ($currpassword == $newpassword) {
                    $this->session->set_flashdata('psn', 'New Passord tidak bisa sama dengan Current Password!');
                } else if ($newpassword != $repeatpassword) {
                    $this->session->set_flashdata('psn', 'New Password harus sama dengan Repeat Password!');
                } else {
                    // $password_hash = password_hash($newpassword, PASSWORD_DEFAULT);
                    $this->db->set('password', $newpassword);
                    // $this->db->set('image', $new_image);
                    //$this->db->set('petugas_id', $petugas_id);
                    $this->db->where('username', $username);
                    $this->db->update('user');

                    $this->session->set_flashdata('msg', 'Data telah diubah');
                }
            }
        } else {
            // $this->db->set('image', $new_image);
            //$this->db->set('petugas_id', $petugas_id);
            $this->db->where('username', $username);
            $this->db->update('user');

            $this->session->set_flashdata('msg', 'Data telah diubah');
        }
        $data= [
            'username' =>$username,
            'image'=> $_FILES['image']['name'],
           
        ];
        $daffa =password_hash($newpassword,PASSWORD_DEFAULT);
        $this->db->set('password', $daffa);
        $this->db->where('id_users', $id_users);
        $this->db->update('user', $data);
        redirect('user/profile_admin');
        
    }
    // SELESAI PROFILE USER PETUGAS

    // MULAI PROFILE USER BIDAN
    public function ubah_profile(){
        $data['title'] = 'Profile | Posyandu Paleto Indah';
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();

        $id_users = $this->input->post('id_users');
        $username = $this->input->post('username');
        $upload_image = $_FILES['image']['name'];
        $currpassword = $this->input->post('current-password');
        $newpassword = $this->input->post('new-password');
        $repeatpassword = $this->input->post('repeat-password');
        if ($upload_image) {
            $config['allowed_types'] = 'gif|jpg|jpeg|png';
            $config['max_size'] = '2048';
            $config['upload_path'] = './build/img/profile';

            $this->load->library('upload', $config);
            if ($this->upload->do_upload('image')) {
                $old_image = $data['user']['image'];
                if ($old_image != 'icon-user.png') {
                    unlink(FCPATH . 'build/img/profile/' . $old_image);
                }

                $new_image = $this->upload->data('file_name');
                // $coba = array(
                //     'image' -> 
                // )
                $this->db->set('image', $new_image);
            } else {
                echo $this->upload->display_errors();
            }
        }
        $pass = $data['user']['password'];

        if ($currpassword !== '') {
            if ($currpassword != $pass) {
                $this->session->set_flashdata('psn', 'Current Password Salah!');
            } else {
                if ($currpassword == $newpassword) {
                    $this->session->set_flashdata('psn', 'New Passord tidak bisa sama dengan Current Password!');
                } else if ($newpassword != $repeatpassword) {
                    $this->session->set_flashdata('psn', 'New Password harus sama dengan Repeat Password!');
                } else {
                    // $password_hash = password_hash($newpassword, PASSWORD_DEFAULT);
                    $this->db->set('password', $newpassword);
                    // $this->db->set('image', $new_image);
                    //$this->db->set('petugas_id', $petugas_id);
                    $this->db->where('username', $username);
                    $this->db->update('user');

                    $this->session->set_flashdata('msg', 'Data telah diubah');
                }
            }
        } else {
            // $this->db->set('image', $new_image);
            //$this->db->set('petugas_id', $petugas_id);
            $this->db->where('username', $username);
            $this->db->update('user');

            $this->session->set_flashdata('msg', 'Data telah diubah');
        }
        $data= [
            'username' =>$username,
            'image'=> $_FILES['image']['name'],
           
        ];
        $daffa =password_hash($newpassword,PASSWORD_DEFAULT);
        $this->db->set('password', $daffa);
        $this->db->where('id_users', $id_users);
        $this->db->update('user', $data);
        redirect('user/profile_admin');
        
    }
    public function profile_admin()
    {
        $data['title'] = 'Profile | Posyandu Paleto Indah';
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();

        // $this->form_validation->set_rules('name', 'Nama Lengkap', 'required|trim');
        // $this->form_validation->set_rules('password1', 'New Password', 'trim|min_length[6]');
        // $this->form_validation->set_rules('password2', 'Repeat Password', 'min_length[6]|matches[passsword1]');

        if ($this->form_validation->run() == false) {
            $this->load->view('templates/header-form', $data);
            $this->load->view('templates/sidebar-admin');
            $this->load->view('templates/topbar-admin', $data);
            $this->load->view('user/profile-admin', $data);
            $this->load->view('templates/footer-form');
        } else {
            $id_users = $this->input->post('id_users');
            $username = $this->input->post('username');
            $upload_image = $_FILES['image']['name'];
            $currpassword = $this->input->post('current-password');
            $newpassword = $this->input->post('new-password');
            $repeatpassword = $this->input->post('repeat-password');

            // if (isset($_POST['submit'])) {                                          
            // var_dump($upload_image);                          
            // die;
            //     //OR
            //     // print_r($_FILES);
            // }
            // die;
            if ($upload_image) {
                $config['allowed_types'] = 'gif|jpg|jpeg|png';
                $config['max_size'] = '2048';
                $config['upload_path'] = './build/img/profile';

                $this->load->library('upload', $config);
                if ($this->upload->do_upload('image')) {
                    $old_image = $data['user']['image'];
                    if ($old_image != 'icon-user.png') {
                        unlink(FCPATH . 'build/img/profile/' . $old_image);
                    }

                    $new_image = $this->upload->data('file_name');
                    // $coba = array(
                    //     'image' -> 
                    // )
                    $this->db->set('image', $new_image);
                } else {
                    echo $this->upload->display_errors();
                }
            }

            $pass = $data['user']['password'];

            if ($currpassword !== '') {
                if ($currpassword != $pass) {
                    $this->session->set_flashdata('psn', 'Current Password Salah!');
                } else {
                    if ($currpassword == $newpassword) {
                        $this->session->set_flashdata('psn', 'New Passord tidak bisa sama dengan Current Password!');
                    } else if ($newpassword != $repeatpassword) {
                        $this->session->set_flashdata('psn', 'New Password harus sama dengan Repeat Password!');
                    } else {
                        // $password_hash = password_hash($newpassword, PASSWORD_DEFAULT);
                        $this->db->set('password', $newpassword);
                        // $this->db->set('image', $new_image);
                        //$this->db->set('petugas_id', $petugas_id);
                        $this->db->where('username', $username);
                        $this->db->update('user');

                        $this->session->set_flashdata('msg', 'Data telah diubah');
                    }
                }
             
            } else {
                // $this->db->set('image', $new_image);
                //$this->db->set('petugas_id', $petugas_id);
                $this->db->where('username', $username);
                $this->db->update('user');

                $this->session->set_flashdata('msg', 'Data telah diubah');
            }
            $data= [
                'username' =>$username,
                'image'=> $_FILES['image']['name']
            ];
            print_r($id_users);
            $this->db->where('id_users', $id_users);
            $this->db->update('user', $data);


            redirect('user/profile_admin');
         }
    }
    // SELESAI PROFILE USER BIDAN

     // MULAI CREATE DATA USER
    public function createDataUser()
    {
        $data = [
            'petugas_id' => $this->input->post('petugas-id'),
            'username' => $this->input->post('username'),
            'password' => password_hash($this->input->post('password1'), PASSWORD_DEFAULT),
            'image' => "icon-user.png",
            'level_id' => $this->input->post('level-id'),
            'is_active' => $this->input->post('is-active'),
            'date_created' => time(),
        ];

        $this->db->insert('user', $data);
        $this->session->set_flashdata('msg', 'Berhasil Ditambahkan');

        redirect('user');
    }
     // SELESAI CREATE DATA USER

     public function updateDataUser($id)
     {
         $data = [
             'petugas_id' => $this->input->post('petugas-id'),
             'username' => $this->input->post('username'),
             'level_id' => $this->input->post('level-id'),
             'is_active' => $this->input->post('is-active'),
         ];
 
         $this->User_model->updDataUser($id, $data);
         $this->session->set_flashdata('msg', 'Berhasil Diubah');
 
         redirect('user/index');
     }

    // MULAI DELETE DATA USER
    public function deleteDataUser($id)
    {
        $this->User_model->delDataUser($id);
        $this->session->set_flashdata('msg', 'Berhasil Dihapus');

        redirect('user/index');
    }
    // SELESAI DELETE DATA USER
}
